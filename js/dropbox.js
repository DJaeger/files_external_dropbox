$(document).ready(function () {
			...
			onCompletion.then(function () {
				...
				if ($(configured).val() == 'true') {
					...
					if (
						...
						$('.configuration').trigger('dropbox_oauth_step2', [{
	...
	$('#externalStorage').on('click', '[name="oauth2_grant_dropbox"]', function (event) {
		...
		if (client_id !== '' && client_secret !== '') {
			$('.configuration').trigger('dropbox_oauth_step1', [{
				backend_id: tr.attr('class'),
				client_id: client_id,
				client_secret: client_secret,
				redirect: location.protocol + '//' + location.host + location.pathname,
	...
	$('.configuration').on('dropbox_oauth_step1', function (event, data) {
		if (data['backend_id'] !== backendId) {
			return false;	// means the trigger is not for this storage adapter
		}

		OCA.Files_External.Settings.OAuth2.getDropboxAuthUrl(backendUrl, data);
	});

	$('.configuration').on('dropbox_oauth_step2', function (event, data) {
 ...
 */
OCA.Files_External.Settings.OAuth2.getDropboxAuthUrl = function (backendUrl, data) {
	var $tr = data['tr'];
	var configured = $tr.find('[data-parameter="configured"]');
 ...
 */
OCA.Files_External.Settings.OAuth2.dropboxVerifyCode = function (backendUrl, data) {
	var $tr = data['tr'];
	var configured = $tr.find('[data-parameter="configured"]');
	var token = $tr.find('.configuration [data-parameter="token"]');
